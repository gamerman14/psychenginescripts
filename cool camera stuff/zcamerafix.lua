--modified to work with aweosme scirpt
...
local cams = {
	...
	border = tableCopy(templateCam),
	out = tableCopy(templateCam)
}
...
function updateCamsEarly(dt)
	if (not dead()) then
		...
		updateCamEarly(cams.border, dt)
		updateCamEarly(cams.out, dt)
	end
...
function updateCams(dt)
	if (not dead()) then
		...
		updateCam(cams.border, dt)
		updateCam(cams.out, dt)
	end
